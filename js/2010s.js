...
for (let i = 0; i < STORE_RECORDS.length; i++) {
    if (STORE_RECORDS[i].category == "2010s") {
...
for (let i = 0; i < addToCartBtn.length; i++) {
    addToCartBtn[i].onclick = () => {
        currentPageCart.push(STORE_RECORDS[i+45]);
        cartPriceCounter += STORE_RECORDS[i+45].price;
        popUpBox.innerHTML =
            `<article id="popUpAlert">
                <table>
                    <tr>
                        <td>Item Added:</td>
                        <td>${STORE_RECORDS[i+45].recordName}</td>
                    </tr>
                    <tr>
                        <td>Item Price:</td>
                        <td>${STORE_RECORDS[i+45].price}$</td>
                    </tr>
                    <tr>
                        <td>Total Price:</td>
...
for (let i = 0; i < addToCartBtn.length; i++) {
    removeFromCartBtn[i].onclick = () => {
        for (let j = 0; j < currentPageCart.length; j++) {
            if (currentPageCart[j] == STORE_RECORDS[i+45]) {
